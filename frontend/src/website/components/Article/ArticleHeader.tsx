...
import DividerC from "../others/MyDivider";

interface IArtcileHeader {
  header: string;
  category: string;
  country: string;
  date: string;
...
const BageCustom: FC<{ badgeWith: number; name: string }> = ({
  badgeWith,
  name,
}) => {
  return (
...
const ArtcleHeader2: FC<IArtcileHeader> = ({
  header,
  category,
  country,
  date,
      ...
      <section>
        ...
        <section className="flex justify-between mx-5 my-2">
          <div>
            ...
            <time
              className="text-gray-500 font-light text-xs sm:text-sm"
              dateTime="12-12-2022"
            >
              {date}
            </time>
...
function ArticleHeader({ header, category, date, country }: IArtcileHeader) {
  return (
    <ArtcleHeader2
      header={header}
      category={category}
      ...
      >
        <div className="sm:p-10">
          <h1 className="uppercase sm:font-extrabold sm:text-6xl z-10 text-orange-theme-dark-gunmetal">
            <span>
              find various kinds of fully-funded
              <span className="text-orange-400"> Scholarship </span>
              including national & Internationaly
            </span>
          </h1>
          <aside className="flex gap-4 mt-2">
            <BageCustom name="Intermediate" badgeWith={80} />
            <BageCustom name="Pakistan" badgeWith={60} />
          </aside>
        </div>
